...
EBFـINSTRUMENTATION = "pass/NoDelay/build/libMemoryTrackPass.so "
...
def processCommandLineArguements():
    global C_FILE, PROPERTY_FILE, STRATEGY_FILE, ARCHITECTURE, category_property,CONCURRENCY, c_file_i, versionInfo, VERSION, OUTDIR, AFL_DIR, PARALLEL_FUZZ
    ...
    parser.add_argument("-s", "--strategy", help="SV-COMP strategy", choices=["kinduction", "falsi", "incr"],
                        default="kinduction")
...
# This function will initialize all the Directories needed for EBF.
def initializeDir():
...
def addGoals(instrumented_file,GoalList):
    ...
    for i in range(len(GoalList)):
        if time.time() < end_time:
            ...
            print("\n\nAdding reach error to the file to generate BMC seeds in ",goal_choice)
...
def runBMCForSeedGenerationONLY(reacherror_CFILE,goal_choice):
    ...
    EBFRunCmd = "python3 " + InputGenerationPath + concurrency_arg + " -p " + PROPERTY_FILE + " -s " + STRATEGY_FILE + " -a " + str(
        ARCHITECTURE) + ' -w ' + witness_DIR_reacherr + " " + reacherror_CFILE + " 1> " + EBF_LOG + SEP + "runCompiReacherrorBMC.log" + " 2> " + EBF_LOG + SEP + "runErrorReacherrorBMC_.log"
    os.system(EBFRunCmd)
...
def runAFL():
    ...
    if category_property=='reach':
        ...
        #preprocessed_c_file3 = "cat  preprocessed2 | sed -e '/^extern int pthread/,/;$/d'  >  preprocessed3 "
        #preprocessed_c_file4 = "cat  preprocessed3 | sed -e '/^extern void pthread/,/;$/d'  >  " + pre_C_File
        #preprocessed_c_file4 = "cat  preprocessed3 | sed -e '1 i\#include <pthread.h>' > " + pre_C_File
        ...
        RunAfl = " AFL_LLVM_THREADSAFE_INST=1 " + AFL_Bin + Optimization + Compile_Flag + EBFـINSTRUMENTATION + pre_C_File + " " + \
                 " -lpthread " + "-L" + EBF_LIB + SEP + " -lmylibFunctions" + ' -o ' + EBF_EXEX + SEP + Executable + " 1> " + EBF_LOG + SEP + "AflCompile.log" + " 2> " + EBF_LOG + SEP + "AflCompileError.log"
        #RunAfl = " AFL_LLVM_THREADSAFE_INST=1 " + AFL_Bin + Optimization   + pre_C_File + " " + \
         #        " -lpthread " + EBF_LIB + SEP + "atomics.c " + EBF_LIB + SEP +"nondet_rand.c " + ' -o ' + EBF_EXEX + SEP + Executable + " 1> " + EBF_LOG + SEP + "AflCompile.log" + " 2> " + EBF_LOG + SEP + "AflCompileError.log"
        os.system(RunAfl)
...
def ParallelFuzzing(inputs):
    global EBF_EXEX, C_FILE, OUTDIR, EBFـINSTRUMENTATION, AFL_DIR, RUN_LOG, TIMEOUT_AFL, start_time, AFL_COMPILER_DIR, preprocessed_c_file, pre_C_File, AFL_Bin, AFL_FUZZ_Bin, AflExexutableFile,found_event,finished_process
...
def witnessFile_pre():
    ...
    for file in os.listdir(Source):
        if file.startswith("witnessInfoAFL1-") or file.startswith("nondetInputs-"):
...
def witnessFile():
    ...
    WitnessRunCmd = "python3 " + witnessFileGeneration + " -p " + PROPERTY_FILE + " -a " + str(
        ARCHITECTURE) + " " + ' ' + C_FILE + ' ' + witness_type + ' -w ' + witness_DIR + ' -l ' + EBF_LOG
