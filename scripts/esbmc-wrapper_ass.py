...
def get_command_line(strat, prop, arch, benchmark, concurrency, dargs):
  global witness_DIR,MEM_LIMIT_ESBMC,BMCtimeout,EXTRA
  ...
  if concurrency:
    command_line += "  --context-bound 2  --compact-trace "
  ...
  if EXTRA:
        for iarg in EXTRA:
            PATHS= PATHS+ " " + iarg
        command_line += f'{PATHS.lstrip()+" "}'
  #else:
     #command_line+='  -I ../pftools3/build/  -I ../pftools3/src/C/include/  '
  ...
  if prop == Property.overflow:
    ...
    if concurrency:
      command_line += "--no-pointer-check --no-bounds-check  --goto-unwind "
    else:
...
parser.add_argument("-t","--timeout", help='ESBMC timeout', action='store',default=30)
